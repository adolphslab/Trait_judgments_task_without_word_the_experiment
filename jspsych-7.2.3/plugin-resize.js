var jsPsychResize = (function (jspsych) {
  ...
  class ResizePlugin {
      ...
      trial(display_element, trial) {
          ...
          html += "</div>";
          // add warning text (hide with the same color as background)
          html += '<div id="resize-btn-warning" style="color: rgb(221,221,221);">Please adjust the box according to the instructions, before continuing!</div>';
          ...
          document.getElementById('jspsych-resize-btn').addEventListener("click", () => {
              if (Math.abs(scale_div.offsetWidth - start_div_width) > 10) {
                  scale();
                  end_trial();
              }
              else {
                  var warning_div = display_element.querySelector("#resize-btn-warning");
